...
def sipp(graph, state_1, state_2, astar=False):
	...
	# First thing to do is to add the query nodes and retrieve their indices:

	start_index, end_index = graph.add_query_points(state_1, state_2)

	# Now we fetch the vector of nodes:
	...
	while pqueue:
		...
		for child_node_index, weight in min_node.edges.items():
			...
			if weight + min_node.gcost < child_node.gcost and not child_node.visited:

				# Update costs for child node:

				child_node.gcost = weight + min_node.gcost
				child_node.hcost = heuristic(child_node.index, end_index)
				child_node.update_fcost()

				# Record its predecessor:

				nodes[child_node.index].predecessor = min_node.index

				# Push newly explored child node into priority queue:
